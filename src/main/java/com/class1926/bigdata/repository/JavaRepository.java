...
public interface JavaRepository extends JpaRepository<Java, Long> {
     ...
     * @return
     */
    @Query(value = "select address as city,AVG(salary) as avgByCity,SUM(hiring) as countByCity,AVG(experience) as experience,SUM(salary) as budget FROM job_java group by address", nativeQuery = true)
